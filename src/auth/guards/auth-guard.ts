...
import { Reflector } from '@nestjs/core';
  ...
  export class AuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> { //diz se a rota pode ser acessada ou nao
      
      const isPublic = this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [ //verifica se a rota esta marcada como public ou nao
        context.getHandler(),
        context.getClass(),
      ]);
      if (isPublic){
      ...
      if (!token) { //se o token nao existir
        throw new UnauthorizedException(); //sabemos ent que o user nao esta autorizado
      }
      try {
        const payload = await this.jwtService.verifyAsync( //se o token eh valido, pegamos o payload (parte de identificacao do user)
          token,
          {
    ...
    private extractTokenFromHeader(request: Request): string | undefined { //recebe a requisicao e retorna uma string caso tenha o token
